        ...
        <div className="flex flex-col gap-6 w-full animate-entrance">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1.5 text-sm">Manage department leads and academic oversight.</p>
                </div>
                                        ...
                                        <div className="flex flex-col gap-1.5">
                                            {supervisor.timing && (
                                                <div className="flex items-center gap-2 text-muted-foreground text-sm">
                                                    ...
                                                    <span className="font-bold text-foreground/80">{supervisor.timing}</span>
                                                </div>
                                            )}
                                            <div className="flex items-center gap-2 text-muted-foreground text-sm">
...
function AddSupervisorDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
    ...
    const addMutation = useMutation({
        ...
        onError: (error) => {
            ...
            console.error("Supervisors Tab Error:", JSON.stringify(error, null, 2));
        }
    });
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[480px] rounded-3xl border-white/10 bg-card/95 backdrop-blur-xl">
                <DialogHeader className="pb-2">
                    ...
                    <p className="text-xs text-muted-foreground font-medium">Add a new department lead to the system.</p>
                </DialogHeader>
                    ...
                    <div className="space-y-3 border-t border-border pt-4">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <Select value={formData.timing || "none"} onValueChange={(val) => setFormData(prev => ({ ...prev, timing: val === "none" ? "" : val }))}>
                                <SelectTrigger className="h-12 rounded-2xl border-white/10 bg-accent/30 text-sm font-medium px-5 transition-all hover:bg-accent/50">
                ...
                <DialogHeader className="pb-2">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-muted-foreground font-medium mt-0.5">Update lead details.</p>
                        </div>
                    </div>
                </DialogHeader>
                <form onSubmit={handleSubmit} className="space-y-4 pt-2">
